...
class TravelExtraction(BaseModel):
    ...
    city: Optional[str] = Field(default=None, description="Hotel city name. Example: Mumbai, Colombo, Bangkok.")
    check_in: Optional[str] = Field(default=None, description="Hotel check-in date in YYYY-MM-DD format. Null if not provided.")
    check_out: Optional[str] = Field(default=None, description="Hotel check-out date in YYYY-MM-DD format. Null if not provided.")
    origin: Optional[str] = Field(default=None, description="Flight origin city or airport code. Example: BOM, CMB, Mumbai.")
    destination: Optional[str] = Field(default=None, description="Flight destination city or airport code. Example: DEL, BKK, Delhi.")
    flight_date: Optional[str] = Field(default=None, description="Flight date in YYYY-MM-DD format. Null if not provided.")
    hotel_id: Optional[str] = Field(default=None, description="ID of the hotel to book. Null if not provided.")
    guest_name: Optional[str] = Field(default=None, description="Guest full name for hotel booking. Null if not provided.")
    guest_email: Optional[str] = Field(default=None, description="Guest email for hotel booking. Null if not provided.")
    room_type: Optional[str] = Field(default=None, description="Hotel room type such as single, double, or suite. Null if not provided.")
    flight_id: Optional[str] = Field(default=None, description="ID of the flight to book. Null if not provided.")
    passenger_name: Optional[str] = Field(default=None, description="Passenger full name for flight booking. Null if not provided.")
    passenger_email: Optional[str] = Field(default=None, description="Passenger email for flight booking. Null if not provided.")
...
def router(state: GraphState) -> dict:
    ...
    try:
        ...
        data = {
            "intent": "unknown", "sub_action": "general",
...
def _format_hotel(hotel: dict) -> str:
    ...
    return f"{name} in {city}, {stars} stars - {currency} {price}/night - {available} rooms"
...
def _format_flight(flight: dict) -> str:
    ...
    origin = origin_data.get("airport", origin_data.get("city", "unknown")) if isinstance(origin_data, dict) else origin_data
    destination = destination_data.get("airport", destination_data.get("city", "unknown")) if isinstance(destination_data, dict) else destination_data
    flight_date = flight.get("flightDate", flight.get("date", flight.get("departure_date", "unknown")))
    ...
    seats = flight.get("availableSeats", flight.get("available_seats", flight.get("seats", "N/A")))
    return f"{airline} {number} from {origin} to {destination} on {flight_date}, {departure_time} - {arrival_time} - {currency} {price} - {seats} seats"
...
async def hotel_node(state: GraphState) -> dict:
    ...
    if state.get("sub_action") == "book":
        ...
        if missing:
            return {
                ...
                "response_text": (
                    ...
                    "Please provide hotel_id, guest_name, guest_email, room_type, check_in, and check_out."
                ),
            }
        ...
        try:
            ...
            return {
                "hotel_results": [], "flight_results": [],
                "response_text": "Hotel booking is temporarily unavailable. Please try again shortly.",
            }

        confirmation = result.get("message") or result.get("status") if isinstance(result, dict) else None
        return {"hotel_results": [], "flight_results": [], "response_text": confirmation or "Hotel booking completed."}
        ...
        try:
            result = await tools["list_hotels"].ainvoke({})
        except Exception:
            return {
                "hotel_results": [], "flight_results": [],
                "response_text": "Hotel search is temporarily unavailable. Please try again shortly.",
...
async def flight_node(state: GraphState) -> dict:
    ...
    if state.get("sub_action") == "book":
        ...
        if missing:
            return {
                "hotel_results": [], "flight_results": [],
                "response_text": "I need more details to book the flight. Please provide flight_id, passenger_name, and passenger_email.",
            }

        try:
            result = await tools["book_flight"].ainvoke({
                "flight_id": flight_id, "passenger_name": passenger_name, "passenger_email": passenger_email,
            })
        except Exception:
            return {
                "hotel_results": [], "flight_results": [],
                "response_text": "Flight booking is temporarily unavailable. Please try again shortly.",
            }

        confirmation = result.get("message") or result.get("status") if isinstance(result, dict) else None
        return {"hotel_results": [], "flight_results": [], "response_text": confirmation or "Flight booking completed."}
        ...
        try:
            result = await tools["list_flights"].ainvoke({})
        except Exception:
            return {
                "hotel_results": [], "flight_results": [],
                "response_text": "Flight search is temporarily unavailable. Please try again shortly.",
...
def unknown_node(state: GraphState) -> dict:
    ...
    try:
        ...
        return {"hotel_results": [], "flight_results": [], "response_text": response.content}
    except Exception as e:
        return {"hotel_results": [], "flight_results": [], "response_text": f"I couldn't understand your request clearly. Error: {str(e)}"}
...
def generate_response(state: GraphState) -> dict:
    ...
    if hotel_results:
        ...
        return {"response_text": f"I found {count} hotel option{'s' if count != 1 else ''}:\n" + "\n".join(lines)}

    if flight_results:
        count = len(flight_results)
        lines = [_format_flight(flight) for flight in flight_results[:5]]
        return {"response_text": f"I found {count} flight option{'s' if count != 1 else ''}:\n" + "\n".join(lines)}
